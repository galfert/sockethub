...
class XMPP {
  ...
  get config() {
    return {
      // platform which requires a persistent connection / websocket to the client(user)
      persist: true
    }
  };
  ...
  __connect(key, credentials, cb) {
    ...
    //     let msg = 'failed connecting ' + credentials.actor['@id'];
    ...
    //   },
    //   online: () => {
    //     this.debug('connected with jid: ' + credentials.actor['@id']);
    //     __removeListeners();
    //     cb(xmpp);
    //   },
    //   close: () => {
    //     // FIXME - not sure in what cases this happens
    //     this.debug('close received for ' + credentials.actor['@id']);
    //     __removeListeners();
    //     throw new Error('received close event for ' + credentials.actor['@id']);
    //   }
    // };
    ...
    // xmpp.on('offline', handlers.close);
    xmpp.on("error", (err) => {
      console.error(err);
    });
